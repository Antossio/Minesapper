...
public class Minesapper extends JFrame implements ActionListener, MouseListener{
	...
	String right="/Minesweeper/right_mine.gif";
	String wrong="/Minesweeper/wrong_mine.gif";
	String ok="/Minesweeper/ok.gif";
	String minePic="/Minesweeper/mine.gif";
	...
	public Minesapper(int x1, int y1, int n1, int minesClick1, int []m, Button []b, int xCord, int yCord){ //конструктор для восстановления сохраненнной игры
		x=x1;
		y=y1;
		n=n1;
		...
		for(int j=45;j<=y*20+25;j=j+20)
		{
			for(int k=20;k<=x*20;k=k+20)
			{
				...
				if(buts[i].status==true)
					buts[i].but.setIcon(new ImageIcon(ok));
				if(buts[i].val!=0&&buts[i].val!=10)
				{
					switch(buts[i].val)  //в зависимости от кол-ва соседних мин ставим цифру и красим ее 
					{
					case 1:buts[i].lab.setForeground(new Color (66,62,220));
					break;
					case 2:buts[i].lab.setForeground(new Color(85,146,48));
					break;
					case 3:buts[i].lab.setForeground(new Color(192,28,63));
					break;
					case 4:buts[i].lab.setForeground(new Color(10,18,76));
					break;
					case 5:buts[i].lab.setForeground(new Color(33,158,174));
					break;
					case 6:buts[i].lab.setForeground(new Color(181,201,11));
					break;
					case 7:buts[i].lab.setForeground(new Color(100,100,50));
					break;
					case 8:buts[i].lab.setForeground(new Color (200,100,100));
					break;
					}
				...
				if(buts[i].val==10)
					buts[i].lab.setIcon(new ImageIcon(minePic));
		...
		for (int z=0; z<buts.length; z++) //подсчет цифр для клеток соседствующих с минами
		{
			...
			if (buts[z].val!=10)
			{
				...
				if(num!=0)
				{
					...
					{
					case 1:buts[z].lab.setForeground(new Color (66,62,220));
					break;
					case 2:buts[z].lab.setForeground(new Color(85,146,48));
					break;
					case 3:buts[z].lab.setForeground(new Color(192,28,63));
					break;
					case 4:buts[z].lab.setForeground(new Color(10,18,76));
					break;
					case 5:buts[z].lab.setForeground(new Color(33,158,174));
					break;
					case 6:buts[z].lab.setForeground(new Color(181,201,11));
					break;
					case 7:buts[z].lab.setForeground(new Color(100,100,50));
					break;
					case 8:buts[z].lab.setForeground(new Color (200,100,100));
					break;
					}
	...
	public void loose(){ //В случае открытия мины открывается все поле
		for (int i=0; i<buts.length; i++)
		{
			...
			if (buts[i].status==true)
				if(buts[i].val==10)
					buts[i].lab.setIcon(new ImageIcon(right));
				else
				{
					buts[i].lab.setText("");
					buts[i].lab.setIcon(new ImageIcon(wrong));
				}
		}
		endGame=true;
		dialogBox("You loose.");	
	...
	public void dialogBox(String s){  //Диалоговое окно для новой игры 
		...
		if(pane==0) 
		{
			int xCord=this.getX();
			int yCord=this.getY();
			new Minesapper(Integer.parseInt(width.getText()),Integer.parseInt(height.getText()),Integer.parseInt(mines.getText()),xCord,yCord);
			this.dispose();
		}
		else if(pane==1)
			this.dispose();
						...
						for (Button tmp:b)
							if(tmp!=null&&tmp.open==false)
							{
								...
								if(tmp.val<10&&tmp.val>0)
									...
									tmp.lab.setIcon(new ImageIcon(minePic));
							}
						int j=0;
						for (Button tmp:b)
							if(tmp!=null)
